...
export const ExamResultForMcqCard = ({ queIndex, question }) => {
    ...
    const submittedMcqOptionId = submittedAnswer?.submitted_mcq_option;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {
                    mcq_questions?.map((option, index) => {
                        ...
                        const isSubmitted = option?.mcq_option_serial === submittedMcqOptionId;

                        let bgColor = "";
                        if (isCorrect) {
                            bgColor = 'bg-green-100 border-green-500';
                        } else if (isSubmitted && !isSubmittedAnsCorrect) {
                            bgColor = 'bg-red-400';
                        }
