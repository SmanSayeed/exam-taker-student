...
export const MTExamHeader = ({ title, modelTestId, packageId }) => {
    ...
    const { data: studentResultData } = useGetSingleStuResultQuery({ studentId: auth.student.id, modelTestId: modelTestId });
                            ...
                            <h1 className="text-2xl font-bold text-gray-800">{title}</h1>
                            {
                                result_summary && (
                                    <Badge variant="outline" className="text-xl font-bold text-gray-800 flex gap-1 items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-trophy mr-1 h-5 w-5" data-sentry-element="Trophy" data-sentry-source-file="page.tsx">
                                        ...
                                        Rank: #{result_summary?.merit_position}
                                    </Badge>
                                )
                            }
                        </div>
